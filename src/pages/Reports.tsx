...
const reportTypes = [
  {
    ...
    description: 'Detailed risk analysis for individual projects',
    icon: FileText,
    iconBg: 'bg-blue-50',
  ...
  },
  {
    ...
    description: 'Overview of entire project portfolio risk',
    icon: PieChart,
    iconBg: 'bg-green-50',
  ...
  },
  {
    ...
    description: 'Summary of detected risks and alerts',
    icon: AlertTriangle,
    iconBg: 'bg-red-50',
...
export default function Reports() {
  ...
  const handleExport = (title: string) => {
    ...
    const highRiskCount = mockProjects.filter((p) => p.riskLevel === 'HIGH' || p.riskLevel === 'CRITICAL').length;
    const totalCost = mockProjects.reduce((sum, p) => sum + p.currentCost, 0);
    ...
    const lines = [
      ...
      `Report: ${title}`,
      `Generated: ${createdAt}`,
      `Filters: Sector=${sectorFilter === 'all' ? 'All' : sectorFilter} | Risk=${riskFilter === 'all' ? 'All' : riskFilter} | Date=${dateFilter || 'All'}`,
      '',
      'PORTFOLIO SUMMARY',
